...
In this setup, **`<QUERY>`** is seamlessly replaced with the user's input query. This method allows us to filter search results based on image characteristics without relying on metadata.
...
- **Intra-category Recommendations:** These suggest items from the same category. For instance, recommending another watch based on a user's preference for a specific watch model.
- **Inter-category Recommendations:** Here, suggestions span different categories. An example might be recommending a couch influenced by a user's affinity for a certain rug and coffee table.
